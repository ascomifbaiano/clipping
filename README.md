...
- **Classificação Heurística Automática**:
...
- **Barra de Acessibilidade Governamental (WCAG 2.1 AAA)**: Controles de alto contraste e redimensionamento dinâmico de fonte com persistência via armazenamento local.
...
- **Tema de Alto Contraste Completo**: Definição de regras CSS específicas para a classe `body.high-contrast` assegurando contraste superior a 7.0:1 (Nível AAA) com fundo preto puro (`#000000`), textos e títulos em amarelo vivo (`#ffff00`) e ciano (`#00e5ff`), e bordas destacadas.
- **Persistência de Preferências**: Armazenamento automático da escala de fonte (`clipping-font-scale`) e do estado de contraste (`clipping-high-contrast`) via `localStorage`, mantendo a configuração ativa entre sessões de navegação.
- **Preservação de Impressão (@media print)**: Ocultação automática da barra de acessibilidade e desativação das regras de alto contraste durante a impressão de relatórios analíticos, garantindo folhas limpas e economia de tinta.
...
- **`clipping_utils.py` v2.0**: Resolver de URL robusto com decodificação Base64 de links do Google News e extração de Tag Canonical `<link rel="canonical">`. Full-Text Content Scan automático em portais `.edu.br` e `.gov.br`.
- **`scraper_clipping.py` v2.0**: Arquitetura Multi-Engine de 4 Camadas com Serper API Organic + News, Google News RSS fragmentado em sub-queries curtas, Bing News RSS e Scraping Direto de 60+ portais locais da Bahia via Serper `site:domínio`. Janela móvel de 7 dias para capturar matérias indexadas com atraso.
...
- **Garantia de Layout**: O `index.html` foi mantido preservado na camada de layout básico.

### 07/08/2026 - Motor de Busca Profunda e Varredura Histórica 2012 (Recuperação de Menções Perdidas)
- Novo Script `scraper_busca_profunda.py`: Motor de recuperação de menções em janela temporal configurável (padrão: 45 dias). Opera em três fases sequenciais com Serper API com filtro de data preciso (Fase 1), Google News RSS com parâmetros after/before (Fase 2) e Bing News RSS como terceiro motor de diversidade (Fase 3).
...
- **Segregação de Abrangência**: Dividida a antiga categoria `'Institucional / Governamental'` em duas categorias distintas: `'Governamental'` (para portais federais, prefeituras e órgãos públicos) e `'Outras Instituições de Ensino'` (para universidades federais como UFBA, e outros Institutos Federais como IFSC, IFSP, IF Sertão PE, etc.).
- **Abas de Segmentação no Frontend**: Adicionadas 3 novas abas no menu superior do dashboard em [index.html](file:///G:/Meu%20Drive/APP/2.%20Projetos%20e%20Aplica%C3%A7%C3%B5es/2.2%20Aplica%C3%A7%C3%B5es%20e%20C%C3%B3digos%20(GitHub)/IF%20Baiano%20APPs/clipping-completo/index.html) (Grande Mídia, Portais Governamentais e Outras Inst. de Ensino). Cada aba possui uma tabela dedicada e paginação dinâmica gerenciada de forma assíncrona por uma rotina JS genérica reutilizável.
